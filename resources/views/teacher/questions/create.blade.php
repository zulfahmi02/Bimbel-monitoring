...
@section('title', 'Tambah Soal - ' . $game->title)
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-1">Tambahkan pertanyaan untuk game "{{ $game->title }}"</p>
    </div>

    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <form action="{{ route('teacher.games.questions.store', $game) }}" method="POST" enctype="multipart/form-data" class="space-y-6" x-data="questionForm()">
            @csrf

            <!-- Question Text -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Pertanyaan</label>
                <textarea name="question_text" rows="3" class="w-full rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" placeholder="Tulis pertanyaan disini..." required>{{ old('question_text') }}</textarea>
                ...
                <div class="space-y-3">
                    <template x-for="(option, index) in options" :key="index">
                        <div class="flex items-center gap-3">
                            <input type="radio" :name="'correct_index'" :value="index" x-model="correctIndex" class="text-primary-600 focus:ring-primary-500 h-4 w-4 cursor-pointer" :checked="index === 0" required title="Pilih sebagai jawaban benar">
            ...
            <div class="border-t pt-6">
                ...
                <input type="number" name="points" value="10" min="1" class="w-24 rounded-lg border-gray-300 focus:border-primary-500 focus:ring-primary-500" required>
